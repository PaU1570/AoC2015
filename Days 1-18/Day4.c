...
int main(int argc, char* argv[] )
{
	...
	while (!fiveZeroes)
	{
		...
		lowestNum++;
		resetMsg(msg);
		sprintf(msg, "%s%d", input, lowestNum);
		printf("%s\n", msg);
		...
		{     
			...
			for (k = 0; k < 4; k++)
			{
				sprintf(hash, "%s%2.2x", hash, u.b[k]);
			}
	...
	printf ("--\tLowest number producing a hash starting with %d zeroes: %d\n--\t", atoi(argv[2]), lowestNum);
	for (int i = 0; i < 32; i++)
		{
			printf("%c", hash[i]);
		}
